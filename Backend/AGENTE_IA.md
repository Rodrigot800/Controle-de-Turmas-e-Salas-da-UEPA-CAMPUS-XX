...
O modelo **não recebe acesso a SQL livre**. Consultas, relatórios e inserções passam por funções com parâmetros, validações e transações. Alterações e exclusões não estão disponíveis nesta primeira versão. Toda inserção pede confirmação no terminal.
